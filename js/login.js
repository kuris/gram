/* 문법아 놀자! - login.html Google 로그인 + 대시보드 (playhanja login.js 참고) */
  ...
  document.addEventListener('gram:auth-changed', function (e) {
    if (e.detail && e.detail.user) {
      ...
      if (w && e.detail.membership && e.detail.membership.isNew) {
        ...
        w.innerHTML = '🎉 <strong>문법아 놀자에 오신 걸 환영해요!</strong> 기존 계정으로 바로 시작했어요. 학습 기록은 이 서비스에만 저장되니 안심하세요.';
      }
      renderDashboard();
    } else { authView.style.display = 'block'; dashView.style.display = 'none'; }
  });
  document.addEventListener('gram:progress-synced', function () {
