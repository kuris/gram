...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (todayBox && concepts.length) {
    ...
    todayBox.innerHTML =
      ...
      '<div class="today-body"><span class="tag" style="background:' + D.categoryColor(c.category) + '22;color:' + D.categoryColor(c.category) + '">' + esc(D.categoryLabel(c.category)) + ' · ' + esc(D.schoolLabel(c.schoolLevel)) + '</span>' +
  ...
  if (todayExp) {
    var M = D.GTELP_MOCK;
    todayExp.innerHTML =
      '<a class="exp-pick" href="mock.html">' +
      '<span class="exp-pick-emoji">🏆</span>' +
      '<span class="mini-q-body"><strong>GTELP 실전 모의고사: 문법 26문항/20분</strong>' +
      '<small>📋 시제 6 · 가정법 6 · 준동사 5 · 조동사 2 · 연결어 2 · 관계사 2 · 당위성 3</small></span><span class="mini-q-go">→</span></a>';
  }
  ...
  if (tipsBox && concepts.length) {
    ...
    tipsBox.innerHTML = tips.map(function (x) {
      return '<div class="tip-card"><span class="tag" style="background:' + D.categoryColor(x.category) + '22;color:' + D.categoryColor(x.category) + '">' + esc(x.title) + '</span><p>💡 ' + esc(x.memoryTip) + '</p></div>';
    }).join('');
  }
  ...
  if (recentBox) {
    try {
      var log = JSON.parse(localStorage.getItem('gram_study_log') || '[]').slice(0, 4);
  ...
  if (counters) {
    counters.innerHTML =
      '<div class="counter"><strong>' + concepts.length + '</strong><span>영문법 개념</span></div>' +
      '<div class="counter"><strong>' + problems.length + '</strong><span>연습 문제</span></div>' +
      '<div class="counter"><strong>' + rules.length + '</strong><span>헷갈리는 표현</span></div>';
  }
});
