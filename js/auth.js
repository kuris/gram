/* ============================================================
   문법아 놀자! - 로그인/진도 동기화 (playhanja auth.js 참고)
   - Google 로그인은 CGAuth + window.sb 사용
   - 이 파일은 gram 전용: service_members service='gram',
     gram 스키마 진도 병합을 담당합니다.
   ============================================================ */
(function () {
  var SERVICE = 'gram';
  ...
  function sdb() { return window.gramDb ? window.gramDb() : sb(); }
  ...
  async function ensureMembership() {
    ...
    try {
      ...
      console.warn('[문법아 놀자] 가입 확인 실패:', e);
      return { isNew: false };
    }
  }
  ...
  async function syncProgressOnLogin() {
    if (!currentUser || syncing || !window.GramProgress) return;
    syncing = true;
    try {
      await window.GramProgress.mergeLocalToCloud();
      document.dispatchEvent(new CustomEvent('gram:progress-synced'));
    } catch (e) {
      console.warn('[문법아 놀자] 진도 동기화 실패:', e);
  ...
  async function init() {
    ...
    try {
      ...
      if (s && s.data && s.data.session) {
        ...
        document.dispatchEvent(new CustomEvent('gram:auth-changed', { detail: { user: currentUser, membership: membership } }));
      }
    } catch (e) {}
    try {
      ...
      window.CGAuth.onChange(function (d) {
        ...
        if (u && (!currentUser || currentUser.id !== u.id)) {
          ...
          ensureMembership().then(function (m) {
            syncProgressOnLogin();
            document.dispatchEvent(new CustomEvent('gram:auth-changed', { detail: { user: currentUser, membership: m } }));
          });
        } else if (!u && currentUser) {
          currentUser = null;
          document.dispatchEvent(new CustomEvent('gram:auth-changed', { detail: { user: null } }));
        }
        renderAuthBox();
      });
    }
