/* ============================================================
   문법아 놀자! - 진도 저장 (localStorage + Supabase 병합)
   - 비로그인: localStorage만 사용
   - 로그인: Supabase gram 스키마에 저장 + 로그인 시 병합
   - playhanja progress.js/auth.js 패턴 참고
   ============================================================ */
(function () {
  var D = window.GramPlayData;
  var LS = {
    concept: 'gram_concept_progress',
    practice: 'gram_practice_progress',
    quiz: 'gram_quiz_results',
    wrong: 'gram_wrong_notes',
    mock: 'gram_mock_results',
    settings: 'gram_settings',
    session: 'gram_last_session'
  };
  ...
  function sanitizeAttempt(conceptId, ok, p) {
    ...
    return {
      ...
      category: c ? c.category : (p ? valid(p.category, D.ALLOWED.category) : null),
      unit: c ? c.unit : null,
      ok: !!ok,
      at: new Date().toISOString()
    };
  }
  function loggedIn() { return !!(window.GramAuth && window.GramAuth.isLoggedIn()); }
  function userId() { var u = window.GramAuth && window.GramAuth.getUser(); return u ? u.id : null; }
  function sdb() { return window.gramDb ? window.gramDb() : null; }
  ...
  function toggleConceptDone(id) {
    ...
    document.dispatchEvent(new CustomEvent('gram:progress-changed', { detail: { id: id, done: !has } }));
    return !has;
  }
  ...
  function recordAttempt(conceptId, ok, p) {
    ...
    var key = (rec.schoolLevel || 'all') + '|' + (rec.category || 'all') + '|' + (rec.unit || 'all');
    ...
    if (loggedIn()) {
      saveStudyLog({ activity_type: 'practice', school_level: rec.schoolLevel, grade_level: rec.gradeLevel, category: rec.category, unit: rec.unit, concept_id: rec.conceptId, count_total: 1, count_correct: ok ? 1 : 0 }).then(function () {}, function () {});
      upsertPracticeProgress(rec.schoolLevel, rec.category, rec.unit, ok).then(function () {}, function () {});
      if (conceptId) upsertConceptProgress(conceptId, false).then(function () {}, function () {});
    }
    document.dispatchEvent(new CustomEvent('gram:progress-changed', { detail: rec }));
  }
  async function upsertPracticeProgress(schoolLevel, category, unit, ok) {
    var db = sdb(), uid = userId();
    if (!db || !uid) return false;
    try {
      var r = await db.from('practice_progress')
        .select('*').eq('user_id', uid).eq('category', category || 'sentence').eq('level', 'normal')
        .maybeSingle();
      ...
      await db.from('practice_progress').upsert({
        user_id: uid, school_level: valid(schoolLevel, D.ALLOWED.schoolLevel), category: valid(category, D.ALLOWED.category) || 'sentence',
  ...
  async function saveQuizResult(r) {
    ...
    var row = {
      ...
      category: r.category ? valid(r.category, D.ALLOWED.category) : null,
  ...
  async function addWrongNote(p, given) {
    var note = {
      ...
      category: p.category || null, unit: p.unit || null, level: p.level || null,
      ...
      try {
        if (db && uid) {
          await db.from('wrong_notes').insert({
            ...
            grade_level: note.gradeLevel, category: valid(note.category, D.ALLOWED.category) || 'sentence',
  ...
  async function saveStudyLog(a) {
    ...
    try {
      await db.from('study_log').insert({
        ...
        category: a.category ? valid(a.category, D.ALLOWED.category) : null,
      ...
      for (var j = 0; j < wrongs.length; j++) {
        ...
        try {
          await db.from('wrong_notes').insert({
            ...
            category: valid(n.category, D.ALLOWED.category) || 'sentence',
  ...
  function stats() {
    ...
    var byCategory = {}, bySchool = {};
    log.forEach(function (l) {
      var f = l.category || 'sentence';
      byCategory[f] = byCategory[f] || { total: 0, correct: 0 };
      byCategory[f].total++; if (l.ok) byCategory[f].correct++;
    ...
    return {
      ...
      byCategory: byCategory, bySchool: bySchool,
