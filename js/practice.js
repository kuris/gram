...
(function () {
  ...
  var catSel = document.getElementById('pr-category');
  ...
  function fillCategories() {
    ...
    catSel.innerHTML = '<option value="all">전체 카테고리</option>' + D.CATEGORIES
      .map(function (c) { return '<option value="' + c.value + '">' + c.emoji + ' ' + c.label + '</option>'; }).join('');
  }
  function fillGrades() {
  ...
  function fillUnits() {
    var list = concepts.filter(function (c) {
      return (schoolSel.value === 'all' || c.schoolLevel === schoolSel.value) &&
             (catSel.value === 'all' || c.category === catSel.value);
    });
  ...
  catSel.addEventListener('change', function () { fillUnits(); fillConcepts(); });
  unitSel.addEventListener('change', fillConcepts);
  fillCategories(); fillGrades(); fillUnits(); fillConcepts();

  var preset = qs('concept');
  if (preset) {
    conceptSel.value = preset;
    var pc = concepts.filter(function (c) { return c.id === preset; })[0];
    if (pc) { schoolSel.value = pc.schoolLevel; fillGrades(); gradeSel.value = pc.gradeLevel; catSel.value = pc.category; fillUnits(); unitSel.value = pc.unit; fillConcepts(); conceptSel.value = preset; }
  }
  ...
  document.getElementById('pr-start').addEventListener('click', function () {
    ...
    var opts = { count: count, level: levelSel.value, category: catSel.value, schoolLevel: schoolSel.value, gradeLevel: gradeSel.value };
  ...
  function check(given) {
    ...
    if (ok) {
      ...
      if (window.GramProgress) window.GramProgress.addWrongNote(p, given);
    }
