/* 문법아 놀자! - progress.html 내 진도 + 오답노트 */
(function () {
  function esc(s) {
    return String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
  }
  var body = document.getElementById('progress-body');
  if (!body || !window.GramProgress) return;
  ...
  function render() {
    var st = window.GramProgress.stats();
    var done = window.GramProgress.getDone();
    var wrongs = window.GramProgress.getWrongNotes();
    ...
    body.querySelectorAll('[data-solve]').forEach(function (b) {
      b.addEventListener('click', function () { window.GramProgress.markWrongSolved(b.getAttribute('data-solve'), true); render(); });
    });
    body.querySelectorAll('[data-del]').forEach(function (b) {
      b.addEventListener('click', function () {
        if (confirm('이 오답을 삭제할까요?')) { window.GramProgress.removeWrongNote(b.getAttribute('data-del')); render(); }
      });
    });
  }
  render();
  document.addEventListener('gram:progress-synced', render);
  document.addEventListener('gram:progress-changed', render);
})();
