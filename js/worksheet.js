/* 문법아 놀자! - worksheet.html 인쇄용 학습지 로직 */
(function () {
  function esc(s) {
  ...
  var catSel = document.getElementById('ws-category');
  ...
  document.getElementById('ws-make').addEventListener('click', function () {
    ...
    var picked = window.GramGenerator.shuffle(list).slice(0, Math.min(count, list.length));
    var problems = [];
    var per = Math.max(1, Math.ceil(count / picked.length));
    picked.forEach(function (c) {
      if (problems.length >= count) return;
      window.GramGenerator.generateByConcept(c.id, Math.min(per, count - problems.length), levelSel.value).forEach(function (p) { problems.push(p); });
    });
    problems = problems.slice(0, count);
    var showAnswer = answerSel.value === 'yes';
    var showSummary = summarySel.value === 'yes';
    var today = new Date().toISOString().slice(0, 10);
    var html = '<div class="sheet"><h2>📝 영문법 학습지 (' + today + ')</h2>' +
